        ...
        <div>
            ...
            @if($parentCategory)
                <nav class="mt-2 text-sm text-gray-600">
                    ...
                    @endif
                    <span class="mx-2">/</span>
                    <span class="text-gray-900">{{ $parentCategory->title }}</span>
                </nav>
            @endif
        </div>
        <a href="{{ route('admin.categories.create', ['level' => $level, 'parent_id' => $parentId]) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 transition-colors">
                    ...
                    @forelse($categories as $category)
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($category->image)
                                    <img src="{{ asset('storage/' . $category->image) }}" alt="{{ $category->title }}" class="w-16 h-16 object-cover rounded-lg">
                                @else
                            ...
                            <td class="px-6 py-4">
                                ...
                                @if($category->parent)
                                    <div class="text-sm text-gray-500">Parent: {{ $category->parent->title }}</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center">
                                ...
                                <div class="mt-6">
                                    <a href="{{ route('admin.categories.create', ['level' => $level, 'parent_id' => $parentId]) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700">
                                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                                        </svg>
                                        Create Category
